...
export default function AIChat() {
  ...
  const [messages, setMessages] = useState<Message[]>([{ role: 'bot', content: "Hi! I'm the TruScan AI. How can I help you automate your business today?" }]);
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      // Pulling the secret key from Vercel
      const apiKey = import.meta.env.VITE_GEMINI_API_KEY;

      if (!apiKey) {
        throw new Error("Key missing! Ensure VITE_GEMINI_API_KEY is set in Vercel.");
      }
    ...
    <>
      ...
      <div className="fixed bottom-8 right-8 z-[100] flex items-center gap-4">
        ...
        <motion.a 
          ...
          whileTap={{ scale: 0.9 }}
          className="w-16 h-16 bg-[#25D366] text-white rounded-full shadow-2xl flex items-center justify-center transition-all hover:brightness-110"
        >
        ...
        <motion.button 
          ...
          className="w-16 h-16 bg-gradient-to-br from-blue-600 to-purple-600 text-white rounded-full shadow-2xl flex items-center justify-center transition-all hover:brightness-110"
        >
          ...
          >
            <div className="p-6 bg-gradient-to-r from-blue-600 to-purple-600 text-white flex justify-between items-center shadow-lg">
              <div className="flex items-center gap-3">
                ...
                <span className="font-bold tracking-tight">TruScan AI</span>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-50 text-sm">
              {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] p-4 rounded-2xl ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-white border border-gray-100 text-gray-800 rounded-tl-none shadow-sm'}`}>
                    {msg.content}
                  </div>
                </div>
              ))}
            ...
            <form onSubmit={handleSend} className="p-4 bg-white border-t border-gray-100 flex gap-2">
              <input 
                ...
                className="flex-1 bg-gray-100 border-none rounded-xl px-4 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500" 
              />
              <button 
                type="submit" 
